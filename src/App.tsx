...
export default function NavigationMenu({ activeApp, setActiveApp }: { activeApp: string; setActiveApp: (app: string) => void }) {
  return (
    <div className="flex gap-4 p-5 bg-white border-2 border-blue-200 rounded-2xl shadow-sm mt-6 mx-4 md:mx-6 lg:mx-8"> 
      <button
        ...
        className={`flex-1 font-bold py-2 px-4 rounded-full transition-colors ${activeApp === "habitTracker" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
        Habit Tracker
      </button>
      <button
        onClick={() => setActiveApp("medicationTracker")}
        className={`flex-1 font-bold py-2 px-4 rounded-full transition-colors ${activeApp === "medicationTracker" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
        Medication
      </button>
      <button
        onClick={() => setActiveApp("heartRateMonitor")}
        className={`flex-1 font-bold py-2 px-4 rounded-full transition-colors ${activeApp === "heartRateMonitor" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
        Heart Rate
      </button>
    </div>    
  );
